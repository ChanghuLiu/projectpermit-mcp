...
class BatchPayload(BaseModel):
    ...
    changes:list[dict[str,Any]]=Field(min_length=1,max_length=25)
...
if PAYMENT_ENFORCED:
    ...
    def assess_change_impact(payload:ChangePayload,ctx:Context):
        '''Paid deterministic Skilled Worker sponsor change-impact preflight.'''
        return invoke(paid_assess,tool_name='assess_change_impact',arguments=payload.model_dump(),ctx=ctx)
    @mcp.tool(annotations=READ)
    def batch_assess_changes(payload:BatchPayload,ctx:Context):
        '''Paid batch change-impact preflight for up to 25 sponsor events.'''
        return invoke(paid_batch,tool_name='batch_assess_changes',arguments=payload.model_dump(),ctx=ctx)
else:
    @mcp.tool(annotations=READ,structured_output=True)
    def assess_change_impact(payload:ChangePayload,ctx:Context)->dict[str,Any]:
        '''Deterministic Skilled Worker sponsor change-impact preflight; x402 disabled in this environment.'''
        return _measured('assess_change_impact',lambda:_assess(payload.model_dump()),billable=True,meta=_meta(ctx))
    @mcp.tool(annotations=READ,structured_output=True)
    def batch_assess_changes(payload:BatchPayload,ctx:Context)->dict[str,Any]:
        '''Batch deterministic sponsor change-impact preflight; x402 disabled in this environment.'''
        return _measured('batch_assess_changes',lambda:_batch(payload.model_dump()),billable=True,meta=_meta(ctx))
