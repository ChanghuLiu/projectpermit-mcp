...
def build_server():
    ...
    server = MCPServer(
        ...
        instructions=(
            ...
            "containing the decision, routing, missing inputs, proposed tasks, deduplicated "
            "official evidence, audit metadata and writeback hints. Preserve unknown facts "
            "rather than guessing them; the engine can return review or municipal-confirmation "
            ...
            "permit filing or inspection approval. For repeated validation, context.client_tag "
            "may be a stable non-sensitive integration label; it is hashed before telemetry. "
            "This public endpoint is a developer-validation preview."
        ),
    )
    ...
    def check_project_requirements(
        ...
        """Use for a proposed renovation/building scope in a supported city when an agent needs permit applicability before quoting, scheduling or design lock. Supply normalized scope facts; use address resolution where supported when property overlays matter. Returns a deterministic determination, confidence, stable rule/version metadata, official-source evidence, workflow guidance and a platform-neutral action bundle for downstream integration. Do not use for final municipal authorization, legal/engineering advice, plan review, permit filing or inspection approval; preserve missing facts instead of guessing."""
    ...
    def check_project_requirements_batch(items: list[dict[str, Any]]) -> dict[str, Any]:
        """Evaluate 1-50 normalized projects in one call. Each item may include client_ref, jurisdiction, project, property/context and optional address/resolve_address. A bad item is isolated instead of failing the whole batch. Returns per-item determinations/workflow/action bundles plus batch-level rule/source freshness audit metadata."""
